...
seterror({...errormessage, [name]: name === "pic"? imageValidators(e): TextValidators(e)})
 ...
 function postdata(e){
  ...
  if(error)
    ...
    let item = MaincategortyStateData.find(x=>x.id !== id && x.name.toLowerCase()===data.name.toLowerCase())
    if(item){
      seterror({...errormessage,name:"Maincategory with This name Already Exist"})
      setshow(true)
      return
    }
...
useEffect(()=>{
  (()=>{
    ...
    if(MaincategortyStateData.length){
let item  = MaincategortyStateData.find(x=>x.id===id)
if(item)
setdata({...data,...item})
else
  navigate("/admin/maincategory")
